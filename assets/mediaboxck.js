/*
mediaboxAdvanced v2.0.3 - The ultimate extension of Slimbox and Mediabox; an all-media script
...
// TODO : à migrer
	...
	$.fn.mediabox = function(_options, linkMapper, linksFilter) {
			...
			var links = this;
			links.map(function() {$(this).attr('mediaboxck_done', '1') });
...
/*  clickBlock code - remove the following three lines to enable right-clicking on links to images  */
	...
	function size() {
		...
		/* if the window gets smaller, it fits the image*/
			...
			centerml = -(centerw/2) - margin;
			centermt = -(centerh/2) - margin;
			center.css({width: centerw, height: centerh, top: top, marginTop: centermt, marginLeft: centerml});
		} else if (
			(mediaInitW >= winWidth-options.imgPadding && (mediaInitH / winHeight) < (mediaInitW / winWidth))
			&& ! center.hasClass('mediaboxckmobile')
			// || (mediaInitH >= winHeight-options.imgPadding && (mediaInitH / winHeight) < (mediaInitW / winWidth))
			// || (mediaWidth < mediaInitW && mediaHeight < mediaInitH)
			) {
	...
	function resizeVideos() {
		...
		if (center.hasClass('mediaboxckmobile')) {
			media.find('object,iframe').height('calc(100% - ' + parseInt(bottom.height()) + 'px)');
		} else {
			media.find('object, iframe').height(media.height());
		}
	}
